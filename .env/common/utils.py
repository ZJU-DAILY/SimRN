...
EnvOutput = collections.namedtuple(
    'EnvOutput', 'reward observation query_result')
...
def init_learner_multi_host(num_training_tpus: int):
  ...
  if tf.config.experimental.list_logical_devices('TPU'): #如果有tpu，本项目不考虑此种情况
...
def init_learner(num_training_tpus):
  ...
  if len(settings.hosts) != 1: # [('/cpu', [device_name])]:shape is (1,2)
...
def validate_learner_config(config, num_hosts=1):
  ...
  assert config.num_envs >= config.inference_batch_size * num_hosts, (
      'Inference batch size is bigger than the number of environments.') #就这么多环境，不能用于推理的数量更多吧
